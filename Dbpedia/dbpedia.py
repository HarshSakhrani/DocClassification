...
fname="/root/DocClassification/DBPedia/dbpedia_csv.tar.gz"
...
dfTrainOriginal=pd.read_csv("/root/DocClassification/DBPedia/dbpedia_csv/train.csv",header=None)
dfTest=pd.read_csv("/root/DocClassification/DBPedia/dbpedia_csv/test.csv",header=None)
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('DBpediaLoss_4.png', bbox_inches='tight')
plt.close()
#plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('DBpediaAcc_4.png', bbox_inches='tight')
plt.close()
...
f=open("DBpedia_4_Results.txt","a")
f.write(str(cm))
f.close()
