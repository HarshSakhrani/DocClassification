...
fname="/root/DocClassification/Yelp/yelp_review_polarity_csv.tar.gz"
...
dfTrainOriginal=pd.read_csv("/root/DocClassification/Yelp/yelp_review_polarity_csv/train.csv",header=None)
dfTest=pd.read_csv("/root/DocClassification/Yelp/yelp_review_polarity_csv/test.csv",header=None)
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('YelpLossPolarity_4.png', bbox_inches='tight')
plt.close()
#plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('YelpAccPolarity_4.png', bbox_inches='tight')
plt.close()
...
f=open("Yelp_4_Polarity_Results.txt","a")
f.write(str(cm))
f.close()
