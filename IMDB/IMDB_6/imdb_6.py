...
df=pd.read_csv("/root/DocClassification/IMDB/IMDB Dataset.csv")
...
encodedLabelListTrain=(labelEncoder.fit_transform(dfTrain['sentiment']))
dfTrain['sentiment']=encodedLabelListTrain

labelEncoder=preprocessing.LabelEncoder()
encodedLabelListTest=(labelEncoder.fit_transform(dfTest['sentiment']))
dfTest['sentiment']=encodedLabelListTest

labelEncoder=preprocessing.LabelEncoder()
encodedLabelListVal=(labelEncoder.fit_transform(dfVal['sentiment']))
dfVal['sentiment']=encodedLabelListVal
...
class Transformer(nn.Module):
  def __init__(self,d_model=768,N=6,heads=8):
    super().__init__()
    self.encoder=Encoder(d_model=768,N=6,heads=8)
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('imdbLoss_6.png', bbox_inches='tight')
plt.close()
#plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('imdbAcc_6.png', bbox_inches='tight')
plt.close()
